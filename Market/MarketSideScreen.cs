...
namespace Market {
    public class MarketSideScreen : SideScreenContent {
        ...
        protected override void OnSpawn() {
            ...
            applyButton.onClick += delegate {
                ...
                foreach (MarketItem item in marketItemsBuffer) {
                    vendingMachineComponent.marketItems.Add(item);
                }
        ...
        public void GenerateStateButtons() {
            ...
            foreach (MarketItem marketItem in marketItems) {
                ...
                if (marketItem.unit != 0) {
                    tooltip += SIDESCREEN.ITEM_TIP_UNIT.Replace("{Unit}", marketItem.unit.ToString());
                }
        ...
        private bool CanAdd(MarketItem item) {
            if ((needConsume + item.price) > vendingMachineComponent.coin){
                return false;
            }
