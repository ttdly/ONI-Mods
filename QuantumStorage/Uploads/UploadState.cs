...
namespace QuantumStorage.Uploads {
  ...
  public class UploadState : KMonoBehaviour {
    ...
    [MyCmpReq] private readonly KBatchedAnimController controller;
    ...
    private void DoUpload() {
      var totalNeedUploadMass = uploadSpeed;
      for (var i = 0; i < storage.items.Count && totalNeedUploadMass > 0; i++) {
        ...
        StaticVar.database.UpdateItems(go.PrefabID(), thisTimeUploadMass,
          (primaryElement.Temperature - targetTemputre) * primaryElement.Element.specificHeatCapacity *
          thisTimeUploadMass);
