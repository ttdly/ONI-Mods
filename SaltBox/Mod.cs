    ...
    public class Patches {
        ...
        public class GeneratedBuildings_LoadGeneratedBuildings_Patch {
            public static void Prefix() {
                LocString.CreateLocStringKeys(typeof(MyStrings.BUILDINGS), "STRINGS.");
        ...
        public class Rottable_Instance_RefreshModifiers_Patch {
            public static void Postfix(Rottable.Instance __instance) {
                if (__instance.master.gameObject.HasTag(StaticVars.StoredInSaltBox)) {
                    Amounts amounts = __instance.master.gameObject.GetAmounts();
                    AmountInstance instance = amounts.Get("Rot");
                    AttributeModifier modifier = new AttributeModifier(instance.amount.Id, 1.2f, MyStrings.MISC.StoreInSaltBoxModifer);
