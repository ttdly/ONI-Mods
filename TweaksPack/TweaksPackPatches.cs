  ...
  public class TweaksPackPatches {
    ...
    public static void ManualPatchBuildings(Harmony harmony) {
      ...
      foreach (var type in types)
        harmony.Patch(type.GetMethod("ConfigureBuildingTemplate"),
          postfix: new HarmonyMethod(typeof(TweaksPackPatches).GetMethod(nameof(FabricatorBuildingPostfix))));
      ...
      foreach (var type in types)
        harmony.Patch(type.GetMethod("ConfigureBuildingTemplate"),
          new HarmonyMethod(typeof(TweaksPackPatches).GetMethod(nameof(BuildingPrefix))));
    ...
    public class EggIncubator_UpdateChore_Patch {
      public static void Prefix(EggIncubator __instance) {
        if (__instance.gameObject.HasTag(TweakableStaticVars.Tags.AutoTweaked)) {
          var chore = (Chore)Traverse.Create(__instance).Field("chore").GetValue();
          chore?.Cancel("NoOp");
          if (__instance.Occupant) __instance.Occupant.GetSMI<IncubationMonitor.Instance>()?.ApplySongBuff();
        }
      }
    }
    ...
    public class OilRefinery_States_InitializeStateMachine_Patch {
      public static void Postfix(OilRefinery.States __instance) {
        ...
        __instance.disabled
          .EventTransition(GameHashes.OperationalChanged, __instance.needResources,
            smi => smi.master.gameObject.AddOrGet<Operational>().IsOperational).Enter(
            delegate(OilRefinery.StatesInstance smi) {
        ...
        __instance.needResources
          .EventTransition(GameHashes.OnStorageChange, __instance.ready,
            smi => smi.master.GetComponent<ElementConverter>().HasEnoughMassToStartConverting()).Enter(
            delegate(OilRefinery.StatesInstance smi) {
              if (smi.master.gameObject.HasTag(TweakableStaticVars.Tags.AutoTweaked)) {
    ...
    public class Compost_States_InitializeStates_Patch {
      public static void Postfix(Compost.States __instance) {
        ...
        __instance.inert
          .EventTransition(GameHashes.OperationalChanged, __instance.disabled,
            smi => !smi.GetComponent<Operational>().IsOperational).PlayAnim("on")
          .ToggleStatusItem(Db.Get().BuildingStatusItems.AwaitingCompostFlip).Enter(
            delegate(Compost.StatesInstance smi) {
              if (smi.master.gameObject.HasTag(TweakableStaticVars.Tags.AutoTweaked)) smi.GoTo(__instance.composting);
            }).ToggleChore(
            delegate(Compost.StatesInstance smi) {
    ...
    public class OilWellCap_AddGasPressure_Patch {
      public static void Postfix(OilWellCap __instance) {
        if (__instance.gameObject.HasTag(TweakableStaticVars.Tags.AutoTweaked) && __instance.NeedsDepressurizing())
          __instance.GetSMI<OilWellCap.StatesInstance>()?.sm.working
            .Set(true, __instance.GetSMI<OilWellCap.StatesInstance>());
      }
    }
    ...
    public class OilWellCap_ReleaseGasPressure_Patch {
      public static void Postfix(OilWellCap __instance) {
        if (__instance.gameObject.HasTag(TweakableStaticVars.Tags.AutoTweaked) &&
            __instance.GetSMI<OilWellCap.StatesInstance>().GetPressurePercent() == 0f)
          __instance.GetSMI<OilWellCap.StatesInstance>()?.sm.working
            .Set(false, __instance.GetSMI<OilWellCap.StatesInstance>());
      }
    }
    ...
    public class Desalinator_StatesInstance_CreateEmptyChore {
      public static void Prefix(Desalinator.StatesInstance __instance) {
        if (__instance.master.gameObject.HasTag(TweakableStaticVars.Tags.AutoTweaked)) {
          ...
          foreach (var item in pooledList) __instance.master.GetComponent<Storage>().Drop(item);
          pooledList.Recycle();
        }
      }
    }
